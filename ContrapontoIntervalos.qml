//   Versão modificada do script Intervalos v.01 de Rogério Tavares Constante  - por Fernando Rauber
//
//   Mudanças:
//   ---------
//
//  - Intervalos compostos são reduzidos a sua forma simples
//  - Intervalos não permitidos são marcados com cores
//  - Output de texto simplificado: somente o essencial é exibido
//
//   segue header original abaixo
...
MuseScore {
      ...
      description: "Contraponto Intervalos.\nPlugin para identificar intervalos em exercícios de contraponto modal a duas vozes Versão modificada do script Intervalos v.01 de Rogério Tavares Constante  por Fernando Rauber. "
      version: "0.1ms232"
...
function mostraIntervalos() {
 ...
 for (var voz1=0;voz1<vozes[1].nota.length;voz1++) { // percorre vozes - 1
  ...
  if (vozes[x].nota[voz1] == 1000 || vozes[x].nota[voz2] == 1000) { var int1 = "erro"; }
      ...
      if (int2 < 0) { 
			 var int1 = tpc2Int(vozes[x].tonal[voz2]-vozes[x].tonal[voz1]); 
			 int2 = int2 * -1; 
			 }
             else { 			 
			 var int1 = tpc2Int(vozes[x].tonal[voz1]-vozes[x].tonal[voz2]);			 
...
//console.log(cursor.tick);
    ...
    var myText = newElement(Element.STAFF_TEXT);
		...
		myText.placement = (Placement.BELOW) - 50;

		if (int1.slice(-1) == "A" || int1.slice(-1) == "D")
		{
			myText.color = "#0000ff";
		}
		
		if (myText.text == '4' || myText.text == '7' || myText.text == '2')
		{
			myText.color = "#0000ff";
